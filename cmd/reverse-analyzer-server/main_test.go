...
func TestWorkflowParametersStoreIntentInsteadOfRuntimeDetails(t *testing.T) {
	s, _ := testServer(t, "")
	params, err := s.workflowParameters("process_injection", map[string]any{
		"instruction":             "在授权进程中增加本地诊断入口",
		"target_process":          "SampleApp.exe",
		"authorization_statement": "这是本地测试程序，已获得所有者授权",
		"constraints":             "只允许当前用户会话，完成后清理",
	})
	if err != nil {
		t.Fatal(err)
	}
	if params["instruction"] == nil || params["target_process"] != "SampleApp.exe" || params["plan_status"] != "required" {
		t.Fatalf("unexpected intent params: %#v", params)
	}
	if _, ok := params["pid"]; ok {
		t.Fatalf("pid must be generated by AI plan: %#v", params)
	}
	if _, ok := params["dll_path"]; ok {
		t.Fatalf("dll path must be generated by AI plan: %#v", params)
	}
}

func TestWorkflowParametersRejectMissingIntent(t *testing.T) {
	s, _ := testServer(t, "")
	if _, err := s.workflowParameters("memory_patch", map[string]any{"pid": 1234, "address": "0x401000"}); err == nil {
		t.Fatal("runtime fields without intent must be rejected")
	}
}

func TestAuthorizedPentestCreationUsesEndpointAndScope(t *testing.T) {
	s, _ := testServer(t, "")
	created := request(t, s, http.MethodPost, "/api/experiments", "", map[string]any{
		"workflow_type":       "authorized_pentest",
		"endpoint":            "https://example.test/login",
		"authorization_scope": "仅测试 example.test/login，禁止压测",
	...
	experiment := payload["experiment"].(map[string]any)
	if experiment["status"] != "queued" {
		t.Fatalf("authorized pentest should stay queued: %#v", experiment)
	}
	params := experiment["metadata"].(map[string]any)["workflow_params"].(map[string]any)
	if params["endpoint"] != "https://example.test/login" || params["authorization_scope"] != "仅测试 example.test/login，禁止压测" {
		t.Fatalf("authorized pentest params missing endpoint or scope: %#v", params)
	}
	if params["context"] != "测试账号已就绪" {
		t.Fatalf("authorized pentest context missing: %#v", params)
	}
}

func TestWorkflowCreationQueuesAIPlanForDynamicPatch(t *testing.T) {
	s, _ := testServer(t, "")
	created := request(t, s, http.MethodPost, "/api/experiments", "", map[string]any{
		"workflow_type":           "memory_patch",
		"target_process":          "SampleApp.exe",
		"authorization_statement": "这是本地测试程序，已获得所有者授权",
		"instruction":             "让程序在点击按钮后保持当前窗口不关闭",
		"constraints":             "只允许当前用户会话，完成后保留回滚",
		"validation_requirements": "记录修改前后证据",
	})
	if created.Code != http.StatusCreated {
		t.Fatal(created.Body.String())
	}
	...
	if experiment["status"] != "awaiting_ai_plan" {
		t.Fatalf("memory patch must wait for AI plan: %#v", experiment)
	}
	params := experiment["metadata"].(map[string]any)["workflow_params"].(map[string]any)
	if params["plan_status"] != "required" || params["instruction"] == nil {
		t.Fatalf("memory patch intent params missing: %#v", params)
	}
	if _, ok := params["pid"]; ok {
		t.Fatalf("runtime pid must not be present before AI plan: %#v", params)
	}
}

func TestWorkflowAIConfirmWritesRuntimeParameters(t *testing.T) {
	s, root := testServer(t, "")
	created := request(t, s, http.MethodPost, "/api/experiments", "", map[string]any{
		"workflow_type":           "process_injection",
		"target_process":          "SampleApp.exe",
		"authorization_statement": "这是本地测试程序，已获得所有者授权",
		"instruction":             "向目标进程添加本地诊断入口",
		"constraints":             "只允许当前用户会话，完成后清理",
		"validation_requirements": "验证进程可见且无残留",
	})
	if created.Code != http.StatusCreated {
		t.Fatal(created.Body.String())
	}
	...
	id := payload["experiment"].(map[string]any)["id"].(string)
	planDir := filepath.Join(root, "experiments", id, "patches", "workflow-ai", "plan-1")
	...
	plan := aiWorkflowPlan{
		ID:             "plan-1",
		ExperimentID:   id,
		Workflow:       "process_injection",
		Status:         "planned",
		Instruction:    "向目标进程添加本地诊断入口",
		TargetIdentity: "SampleApp.exe",
		LocationBasis:  "通过授权目标进程与运行中的窗口标题匹配",
		PID:            4321,
		DLLPath:        filepath.Join(root, "mods", "agent.dll"),
		Method:         "load_library",
		Evidence:       []string{"目标进程已确认", "样本位于工作区"},
		Validation:     []string{"确认注入后窗口显示诊断入口"},
		Risks:          []string{"目标进程退出会终止会话"},
		Rollback:       []string{"调用清理流程并移除注入模块"},
		Provider:       "rule_based",
		Model:          "local",
		CreatedAt:      now(),
		UpdatedAt:      now(),
	}
	if err := os.MkdirAll(filepath.Dir(plan.DLLPath), 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(plan.DLLPath, []byte("MZ"), 0600); err != nil {
		t.Fatal(err)
	}
	if err := writeFileJSON(filepath.Join(planDir, "plan.json"), plan); err != nil {
		t.Fatal(err)
	}
	if _, err := s.mutateExperiment(id, func(current *Experiment) error {
		current.Metadata["workflow_ai_plan"] = map[string]any{"id": "plan-1", "status": "planned", "provider": "rule_based", "model": "local"}
		current.Status = "planned"
		return nil
	}); err != nil {
		t.Fatal(err)
	}
	response := request(t, s, http.MethodPost, "/api/experiments/"+id+"/workflow/ai-confirm", "", map[string]any{"plan_id": "plan-1", "confirmation": aiWorkflowPlanConfirmation})
	if response.Code != http.StatusOK {
		t.Fatalf("ai confirm status=%d body=%s", response.Code, response.Body.String())
	}
	current, err := s.loadExperiment(id)
	if err != nil {
		t.Fatal(err)
	}
	params := current.Metadata["workflow_params"].(map[string]any)
	if params["plan_status"] != "confirmed" || params["pid"] != float64(4321) {
		t.Fatalf("runtime params not written from confirmed plan: %#v", params)
	}
	if params["dll_path"] == nil || params["method"] != "load_library" {
		t.Fatalf("confirmed plan missing dll runtime fields: %#v", params)
	}
}
