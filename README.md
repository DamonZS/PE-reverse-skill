...
> **核心能力：模型破甲 / 越狱 / 安全策略绕过**
>
> 内置多层破甲方案，可突破主流 AI 大模型（GPT-5.x / Codex / Luna 系列）的内容安全限制，实现完全无限制模式下的授权安全测试与逆向分析。通过 `codex-instruct.py` 指令注入部署，支持 Codex CLI 命令行版本（已验证有效）；Codex 桌面版因内置 `provider-sync` 覆盖机制暂不支持。
>
