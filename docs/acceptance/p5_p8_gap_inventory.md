...
| P7 | Graphics/ImGui, overlays, UIA/VLM, source reconstruction, target control | UIA bounded scope done; VLM production adapter and opt-in fixture implemented; graphics Present, ImGui and combined matrix/projection/GDI overlay have registered retained-acceptance entry points | Retained OpenAI-compatible VLM endpoint/image evidence; execute the graphics fixtures against a production bridge and controlled graphics host; live control backends |
