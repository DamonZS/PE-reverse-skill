...
| P6 | Protocol analysis and runtime replay | offline analysis and the bounded IPv4/IPv6 loopback TCP/UDP, HTTP/1.1, verified TLS, mutation, ordered session replay, bounded CONNECT capture, and cleartext prior-knowledge HTTP/2 capture/replay are implemented; general capture/replay remains partial | Arbitrary interfaces, unmanaged TLS sessions, generalized CONNECT replay, HTTP/2 over TLS/ALPN, HTTP/3, unrestricted remote endpoints, and broad cross-transport replay |
