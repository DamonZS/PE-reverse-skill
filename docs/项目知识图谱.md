...
| 跨平台指令部署 | `reverse_analyzer/instructions/`（`adapter.py`、`platforms/*`、`registry.py`、`cli.py`） | 把品牌化「拓扑」破甲指令注入 Codex / Claude / Cursor / WorkBuddy 四平台；内容层复用 `topo-ac-unrestricted` 主弹药，平台无关。 |
...
7. 跨平台指令部署：从 `reverse_analyzer/instructions/platforms/` 看四平台注入机制差异，用 `reverse-instruct describe --platform <p>` 查看只读计划，再用 `deploy --dry-run` 预览、`deploy --allowed` 授权写入；细节见 [跨平台破甲指令部署指南](跨平台破甲指令部署指南.md)。
