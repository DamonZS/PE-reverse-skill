...
description: Master-first, configuration-driven reverse-analysis routing for local files, endpoint descriptors, interfaces, package ecosystems, and controlled protection reviews. It returns an evidence-oriented workflow with indexed subskills and local helper tools without executing a target, fetching a URL, or installing dependencies.
---
...
declared local helper tools. It never executes a target, calls a provider,
fetches an endpoint, or installs a dependency.
...
or package ecosystem. An endpoint is only classified; the router does not send
a request to it.
...
2. Use the returned subskills and tool records as an index; do not infer or run undeclared helpers.
...
| License, integrity, anti-tamper, anti-cheat | `protection-review` | evidence review and `anti_tamper_lab` as a candidate only |
| EDR, endpoint telemetry, detection coverage | `edr-defense-review` | defensive static analysis and case review |

## Controlled Protection Reviews

License, anti-tamper, anti-cheat, and EDR inputs are routed only when the
request includes explicit protection evidence such as intent keywords, an
interface kind, or a package descriptor. Their plans set
`authorization_required` and expose only review, evidence, and defensive
analysis stages. They do not automate a license bypass, tampering, injection,
unhooking, evasion, disablement, or anti-cheat bypass.

## Operating Boundary

- Keep the default pass local and offline.
- Use only helpers declared by the route and tool manifest; report missing
  dependencies rather than installing them.
- Treat `--endpoint` and `--url` as strings for classification. Do not fetch
  them from this workbench.
- Do not execute, emulate, debug, patch, inject into, or attach to a target
  from this route layer.
- Label conclusions as observed, inferred, or unresolved and retain an
  evidence path for each conclusion.
