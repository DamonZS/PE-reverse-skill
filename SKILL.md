...
description: "通用全平台逆向分析工具，覆盖 Windows PE/EXE/DLL、Android APK、iOS IPA 三大平台，以及 API 接口逆向和 Web 主动攻击。支持从静态分析到加壳检测到脱壳到反编译到源码到修改源码到重构建完整链路，内含 12 模块 Web 主动攻击引擎。"
triggers:
  - 逆向
  - 脱壳
  - 反编译
...
用户触发技能
    ...
    │
    ├─ 引导模式：判断用户意图
    │   ├─ "我有个 EXE/DLL 要逆向"     → 阶段 1：静态分析
    │   ├─ "我要脱壳"                  → 阶段 2：加壳检测与脱壳
    │   ├─ "我有脱壳后的文件要反编译"   → 阶段 3：深度反编译
    │   ├─ "我要把逆向结果变成源码"     → 阶段 4：源码重构
    │   ├─ "我要分析 APK"             → APK 分析流程
    │   ├─ "我要分析 IPA"             → IPA 分析流程
    │   ├─ "我要审计 Web/API 安全"     → Web API 安全审计
    │   ├─ "我要用 AI 辅助逆向"        → Codex AI 辅助逆向
    │   └─ "不确定/帮我看看"           → 全流程概览
    │
    └─ 全流程概览：展示工作流总览图 + 询问目标文件
```
...
| 用户表达 | 自动路由到 | 首条命令 |
|---------|-----------|---------|
| "逆向这个 EXE" / "分析程序" | 阶段 1 | `pe_analyze.py <target> --deep` |
| "脱壳" / "有壳" / "UPX/VMP/Themida" | 阶段 2 | `suspend_dump.py <packed> --output <unpacked>` |
| "反编译" / "伪代码" / "函数列表" | 阶段 3 | `deep_decompile.py <unpacked> --output ./deep_analysis/` |
| "重构源码" / "还原代码" / "可编译" | 阶段 4 | `reconstruct.py <target> --output ./reconstructed/` |
| "APK 分析" / "安卓逆向" | APK 流程 | `apk_analyze.py <target.apk>` |
...
当用户触发技能但未给出明确目标时，使用以下模板引导：

```
已加载 pe-reverse-analyzer 技能。我可以帮你做：

📦 PE/EXE/DLL 逆向分析 → 静态分析 → 脱壳 → 反编译 → 源码重构
📱 APK 逆向分析 → 反编译 → Java 源码 → Android Studio 项目
...
对于不熟悉命令行的用户，引导其使用交互式向导：

```bash
# Codex AI 交互式向导（无需记忆命令行参数）
...
| `codex_helper.py` | **Codex AI 辅助逆向引擎** | standard library | ✅ 新增 |
...
## Codex AI 辅助逆向引擎 🧠🆕 (整合自 Codex-5.5-codex-instruct)

### 核心能力

将 GPT-5.5 无限制模式集成到逆向工作流中，在关键分析节点引入 AI 辅助：
...
```bash
# 阶段 1 后：将静态分析结果喂给 AI 深度解读
python scripts/codex_helper.py prompt \
    --type pe-analyze \
    --target target.exe \
    --analysis analysis.json \
    --output ai_analysis_prompt.md

# 阶段 3 后：将伪代码喂给 AI 做关键函数识别和算法还原
python scripts/codex_helper.py prompt \
    --type decompile \
    --target unpacked.exe \
    --pseudocode-dir ./pseudocode/ \
    --max-funcs 30 \
    --output ai_decompile_prompt.md
...
python scripts/codex_helper.py prompt \
    ...
    --analysis analysis.json \
    --pseudocode-dir ./pseudocode/ \
    --output ai_reconstruct_prompt.md

# Web 审计后：让 AI 构造攻击链和修复方案
python scripts/codex_helper.py prompt \
    --type web-audit \
    --target https://example.com \
    --report web_attack_report.md \
    --output ai_webaudit_prompt.md

# API 逆向：让 AI 生成 SDK 和 OpenAPI 规范
python scripts/codex_helper.py prompt \
    --type api-reverse \
    --target flow.xml \
    --output ai_apireverse_prompt.md

# 自定义分析请求
python scripts/codex_helper.py prompt \
    --type custom \
    --target target.exe \
    --custom-prompt "分析这个程序的加密算法，给出 C 语言实现" \
    --output ai_custom_prompt.md
```

### AI 辅助逆向工作流

```
阶段 1: pe_analyze.py → analysis.json
    │
    ├─→ codex_helper.py prompt --type pe-analyze
    │   └─ AI 输出: 程序画像、壳评估、功能模块推断、重构路线图
    │
阶段 2: suspend_dump.py → unpacked.exe
    │
阶段 3: deep_decompile.py → pseudocode/
    │
    ├─→ codex_helper.py prompt --type decompile
    │   └─ AI 输出: 关键函数识别、调用链、IAT 映射、算法还原
    │
阶段 5: integrate_v2.py → reconstructed/
    │
    ├─→ codex_helper.py prompt --type reconstruct
    │   └─ AI 输出: 可编译 C 实现、CMakeLists.txt、编译注意事项
    │
阶段 6: 人工 + AI 协作 → final_project/
```

### 注意事项

- **部署前务必备份**：`codex_helper.py deploy` 会自动备份 config.toml，但建议手动确认
- **AI 输出仅供参考**：AI 生成的代码需要人工审查和编译验证
- **上下文截断**：大型分析数据会被截断到 8000 字符，伪代码默认取前 20 个函数
- **恢复方法**：`python scripts/codex_helper.py restore` 可从最近备份恢复配置
- **无额外依赖**：仅使用 Python 标准库，不需要安装任何 pip 包
